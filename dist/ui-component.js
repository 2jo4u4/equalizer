...
export class EqualizerUIComponent {
    ...
    reset() {
        this.sliderList.forEach(({ input }) => {
            ...
            input.dispatchEvent(new InputEvent("input", { data: input.dataset.init }));
        });
    }
    /**
     * @param init 初始值
    ...
    addSilder(init, max, min, step, option) {
        const initStr = init.toString(), maxStr = max.toString(), minStr = min.toString(), stepStr = step.toString(), input = EqualizerUIComponent.sliderInput(), container = EqualizerUIComponent.sliderContiner();
        input.max = maxStr;
        input.min = minStr;
        input.step = stepStr;
        input.value = initStr;
        input.dataset.init = initStr;
        const { target, filter, addToDashBoard, fliterType, f, q, g } = Object.assign({
            target: "gain",
            addToDashBoard: true,
            filter: undefined,
        ...
        const titleText = document.createElement("span"), maxText = document.createElement("span"), minText = document.createElement("span");
